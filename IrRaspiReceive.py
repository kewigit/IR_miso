...
./irrp.py -r -g4 -fcodes 1 2 3 4 5 6　受信時
...
./irrp.py -p -g17 -fcodes 2 3 4　送信時
...
def backup(f): #Fileすでにがあれば[FileName].bakを作る
   """
...
if True: # Record.メイン受信
   ...
   print("GLITCH = ",GLITCH)
   print("PRE_MS = ",PRE_MS)
   print("POST_MS = ",POST_MS)
   print("FREQ = ",FREQ)
   print("VERBOSE = ",VERBOSE)
   print("SHORT = ",SHORT)
   print("GAP_MS = ",GAP_MS)
   print("NO_CONFIRM = ",NO_CONFIRM)
   print("TOLERANCE = ",TOLERANCE)
   print("POST_US = ",POST_US)
   print("PRE_US = ",PRE_US)
   print("GAP_S = ",GAP_S)
   print("CONFIRM = ",CONFIRM)
   print("TOLER_MIN = ",TOLER_MIN)
   print("TOLER_MAX = ",TOLER_MAX)
   print("***********************")

   while 0 < ammunition:

         print("Press key for '{}'".format("Infrared"))
         print("弾数は",ammunition,"弾です")
         code = []
         fetching_code = True
         while fetching_code:
            time.sleep(0.1)
         ...
         if CONFIRM:
            ...
            tries = 0
            while not done:
               print("Press key for '{}' to confirm".format("Infrared"))
               code = []
               fetching_code = True
               while fetching_code:
                  time.sleep(0.1)
               ...
               if the_same:
                  ...
                  if tries <= 3:
                     ...
                     print("Giving up on key '{}'".format("Infrared"))
                     done = True
                  time.sleep(0.5)
         else: # No confirm.
            print("赤外線検知")
            ammunition -= 1
            print("残り弾数",ammunition,"弾です")
